...
class WebhookHandler
{
    ...
    public function handleIncomingMessage($body)
    {
        ...
        if ($messageType === 'text') {
            ...
            if (trim($textBody) === '#testflow') {
                $this->sendLoginFlow($nomorPengirim);
                return;
            }
        ...
        if ($message['type'] === 'interactive' && isset($message['interactive']['type']) && $message['interactive']['type'] === 'nfm_reply') {
            $this->processFlowReply($message, $conversation, $namaPengirim);
            return;
        }
        if ($message['type'] === 'interactive' && isset($message['interactive']['type']) && $message['interactive']['type'] === 'button_reply') {
    ...
    private function processFlowReply($message, $conversation, $namaPengirim)
    {
        $nomorPengirim = $message['from'];
        $responseJson = $message['interactive']['nfm_reply']['response_json'];
        $body = $message['interactive']['nfm_reply']['body'];
        $flowData = json_decode($responseJson, true);
        $this->logger->info("Flow Response Received: " . $responseJson);
        $flowToken = $flowData['flow_token'] ?? '';
        if ($flowToken === 'TOKEN-LOGIN-TEST') {
            $email = $flowData['email'] ?? '-';
            $password = $flowData['password'] ?? '-';
            $pesanBalasan = "✅ *Data Flow Diterima!*\n\nEmail: $email\nPassword: $password";
            kirimPesanTeks($nomorPengirim, $pesanBalasan);
            $this->conversationService->saveMessage($conversation['id'], 'user', 'text', "[Flow Login] Email: $email");
            $this->saveAdminReply($conversation['id'], $nomorPengirim, $pesanBalasan, 'text');
        } else {
            kirimPesanTeks($nomorPengirim, "Flow token tidak dikenali.");
        }
    }
    private function sendLoginFlow($nomorPengirim)
    {
        $flowId = Config::get('WHATSAPP_FLOW_LOGIN_ID');
        $screenId = "SIGN_IN";
        $judulHeader = "Alo aku edo";
        $pesanBody = "Test Edo";
        $pesanFooter = "Asoka Baby Store";
        $namaTombol = "Test lagii";
        $dataAwal = [];
        $result = kirimPesanFlow(
            $nomorPengirim,
            $flowId,
            $screenId,
            $judulHeader,
        ...
        if ($result['success']) {
            $this->logger->info("Login Flow sent successfully to $nomorPengirim");
        } else {
            $this->logger->error("Failed to send Login Flow: " . $result['response']);
        }
    }
}