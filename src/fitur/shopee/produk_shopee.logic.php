...
if ($shopeeService->isConnected()) {
    ...
    $stmt_cnt->close();
    $sql_data = "SELECT sp.* " . $sql_core . $joins . $wheres . " ORDER BY sp.nama_produk ASC LIMIT ?, ?";
    ...
    while ($row = $result_data->fetch_assoc()) {
        ...
        $item = [
            ...
            'stock_info_v2' => [
                'summary_info' => [
                    'total_available_stock' => (int) $row['stok']
                ]
            ],
            'calculated_total_stock' => (int) $row['stok'],
            'models' => []
        ];
        if ($is_variant) {
            $model = [
            ...
            $item['models'][] = $model;
            $item['matching_models'] = $item['models'];
            if (!empty($row['sku']))
                $all_skus[] = $row['sku'];
        } else {
            if (!empty($row['sku']))
