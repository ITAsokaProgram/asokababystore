...
try {
    ...
    $logger->info("[SUCCESS] Panggilan API sync_all_products_to_redis selesai. Total: {$total_products}. Durasi: {$duration} detik.");
    echo json_encode([
        'success' => true,
    ...
    if (strpos($errorMessage, 'Sinkronisasi sudah sedang berjalan') !== false) {
        $statusCode = 429;
    } elseif (strpos($errorMessage, 'Not authenticated') !== false || strpos($errorMessage, 'Token Shopee tidak valid') !== false) {
        $statusCode = 401;
