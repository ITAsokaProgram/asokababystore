...
$logger = new AppLogger('shopee_FORCE_sync_db.log');
try {
...
try {
    ...
    $total_products = $shopeeService->syncAllProductsToDatabase($conn, true);
    $endTime = microtime(true);
    $duration = round($endTime - $startTime, 2);
    $logger->info("[SUCCESS] FORCE Sync DB selesai. Total: {$total_products}. Durasi: {$duration}s.");
    echo json_encode([
        'success' => true,
        'message' => 'Sinkronisasi PAKSA produk ke Database selesai.',
    ...
    $endTime = microtime(true);
    $duration = round($endTime - $startTime, 2);
    $logger->critical("🔥 FATAL ERROR API: " . $t->getMessage());
    if (!headers_sent()) {
