...
$logger = new AppLogger('top_customer_cabang.log');
try {
...
try {
    ...
    $join_store = " JOIN kode_store ks ON t.kd_store = ks.Kd_Store ";
    $where_cabang = " AND ks.display = 'on' ";
    $params = $dateFilter['params'];
    $types = $dateFilter['types'];
    if (!empty($cabang_filter) && $cabang_filter !== 'all') {
        $where_cabang .= " AND ks.Nm_Alias = ? ";
