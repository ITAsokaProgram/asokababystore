...
try {
    ...
    $sql_data = "
        SELECT
            ...
            SUM(ifnull(a.ppn,0)*(case when a.timbang='true' then a.qty_rec/1000 else a.qty_rec end)) as ppn,
            SUM(
                ...
                (ifnull(a.ppn_bm,0)*(case when a.timbang='true' then a.qty_rec/1000 else a.qty_rec end))
            ) as total
        FROM
            receipt a
        LEFT JOIN
