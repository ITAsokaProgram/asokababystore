...
// Menambahkan middleware auth untuk mendapatkan identitas user
...
try {
    ...
    // --- BAGIAN YANG DIGANTI: PENGAMBILAN KODE STORE SESUAI USER ---
    $sqlUserCabang = "SELECT kd_store FROM user_account WHERE kode = ?";
    $stmtUser = $conn->prepare($sqlUserCabang);
    $stmtUser->bind_param("s", $verif->kode);
    $stmtUser->execute();
    $resUser = $stmtUser->get_result();

    if ($resUser->num_rows > 0) {
        $userCabang = $resUser->fetch_assoc();
    ...
    $stmtUser->close();
    // --- AKHIR PERUBAHAN PENGAMBILAN KODE STORE ---
