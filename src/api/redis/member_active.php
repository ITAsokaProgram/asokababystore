...
$aktif = array_filter($data['data'], function ($item) {
    return isset($item['status_aktif']) && strcasecmp($item['status_aktif'], 'Aktif') === 0;
});

// Simpan hasil filter ke Redis
$redis->setex('member_active', $ttl, json_encode(array_values($aktif)));

echo date('Y-m-d H:i:s') . " - Redis updated: member_active (" . count($aktif) . " member aktif)\n";
