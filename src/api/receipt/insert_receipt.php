...
try {
    ...
    if (empty($kode_store) || empty($tgl_receipt) || empty($kode_supp) || empty($no_faktur)) {
    ...
    $sql_insert = "INSERT INTO c_receipt 
                   (tgl_receipt, kode_store, kode_supp, nama_supplier, no_faktur, no_invoice, total_penerimaan, keterangan) 
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
