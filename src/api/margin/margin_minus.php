...
if ($kd_store == "Pusat") {
    ...
    t.kd_store AS kode,
    m.status_cek,
    m.nama_cek,   /* Fixed: Ditambahkan agar status cek terbaca */
    m.ket_cek,    /* Fixed: Ditambahkan agar status cek terbaca */
    SUM(
        CASE 
    ...
    FROM (
        SELECT 
            ...
            tr.conv2, tr.conv1, tr.qtyppn, tr.tgl_trans, tr.kd_store, tr.no_bon
        FROM trans_b tr
        WHERE tr.tgl_trans = CURDATE() - INTERVAL 1 DAY AND tr.kd_store IN ($placeholders)
        AND tr.avg_cost > tr.hrg_promo
    ) AS t 
    LEFT JOIN kode_store s ON t.kd_store = s.kd_store
    LEFT JOIN margin m ON t.plu = m.plu AND m.kd_store = t.kd_store AND m.no_bon = t.no_bon
