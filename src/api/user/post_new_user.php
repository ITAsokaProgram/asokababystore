...
try {
    ...
    $stmt = $conn->prepare("
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
    // Perhatikan: $position masuk ke kolom 'Hak'. Karena DB Hak varchar(50), 
    // pastikan value option di HTML tidak lebih dari 50 char.
    // 'Store Manager / Asisten Store Manager' adalah 35 karakter, jadi aman.
    ...
    if (!empty($menus)) {
        ...
        // Bind parameter di luar loop untuk efisiensi
        ...
        foreach ($menus as $menu) {
            ...
            // Ambil endpoint url berdasarkan menu_code
            ...
            // Jika menu code tidak ada di database menu_website, skip
            if (!$row) {
                continue;
            }
