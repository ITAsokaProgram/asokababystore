    ...
    try {
      if (
        ...
        const isModalVisible =
          currentSwal &&
          currentSwal.style.display !== "none" &&
          !currentSwal.classList.contains("swal2-toast");

        if (
          (data.event === "new_live_chat" || data.event === "new_message") &&
          data.conversation_id !== currentConversationId &&
          !isModalVisible
        ) {
          const title =
            data.event === "new_live_chat" ? `Live chat baru` : `Pesan baru`;
        ...
        if (data.event === "new_live_chat") {
          ...
          let existingItem = listElement.querySelector(
            `.conversation-item[data-id="${data.conversation_id}"]`
          );

          if (existingItem) {
            const liveBadge = existingItem.querySelector(".live-badge");
            if (!liveBadge) {
              const titleContainer =
                existingItem.querySelector(".font-semibold")?.parentElement;
              if (titleContainer) {
                const newBadge = document.createElement("span");
                newBadge.className =
                  "live-badge px-2 py-0.5 text-xs font-semibold text-red-700 bg-red-100 rounded-full flex-shrink-0 shadow-sm";
          ...
          if (data.conversation_id === currentConversationId) {
            ...
            fetch(
              `/src/api/whatsapp/get_cs_data.php?conversation_id=${data.conversation_id}`,
              {
            ...
            if (existingItem) {
              ...
              if (!unreadBadge) {
                ...
                unreadBadge.className =
                  "unread-badge bg-blue-500 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-md";
                const timeElement = existingItem.querySelector(
        ...
        } else if (data.event === "conversation_status_update") {
          if (data.conversation_id === currentConversationId) {
            currentConversationStatus = data.new_status;
          ...
          if (currentConvoPage === 1 && currentSearchTerm === "") {
            ...
            if (existingItem && data.new_status !== "live_chat") {
              const liveBadge = existingItem.querySelector(".live-badge");
          ...
          if (wamid && status) {
            const messageBubble = document.querySelector(
              `.message-bubble[data-wamid="${wamid}"]`
            );
            if (messageBubble) {
              const statusIconElement = messageBubble.querySelector(
                ".message-status-icon"
              );
              if (statusIconElement) {
                const newIconHTML = getStatusIcon(status);
                statusIconElement.outerHTML = newIconHTML;
              }
            }
          }
        }
...
async function fetchAndRenderConversations(isLoadMore = false) {
  try {
    ...
    const response = await fetch(
      `/src/api/whatsapp/get_cs_data.php?${params.toString()}`,
      {
        headers: { Authorization: `Bearer ${wa_token}` },
      }
    );
    ...
    if (data.unread_counts) {
      updateFilterUnreadBadges(data.unread_counts);
      const totalUnread =
        (data.unread_counts.live_chat || 0) + (data.unread_counts.umum || 0);
    ...
    conversations.forEach((convo) => {
      ...
      item.className =
        "conversation-item p-4 border-b border-gray-100 cursor-pointer hover:bg-blue-50 transition-all duration-200 relative";
...
async function fetchAndUpdateBadges() {
  try {
    const response = await fetch(
      `/src/api/whatsapp/get_cs_data.php?counts_only=true`,
      {
        headers: { Authorization: `Bearer ${wa_token}` },
      }
    );
...
async function selectConversation(conversationId, clickedItemElement = null) {
  ...
  if (!currentItemElement && conversationId) {
    currentItemElement = document.querySelector(
  ...
  try {
    const response = await fetch(
      `/src/api/whatsapp/get_cs_data.php?conversation_id=${conversationId}&page=1`,
      {
        headers: { Authorization: `Bearer ${wa_token}` },
      }
    );
    ...
    document.getElementById("chat-with-phone").textContent =
      details.nomor_telepon;
    document.getElementById("chat-with-name").textContent =
      details.nama_profil ?? "-";
    document
      .getElementById("edit-display-name-button")
...
async function loadMoreMessages() {
  ...
  try {
    const response = await fetch(
      `/src/api/whatsapp/get_cs_data.php?conversation_id=${currentConversationId}&page=${currentMessagePage}`,
      {
        headers: { Authorization: `Bearer ${wa_token}` },
      }
    );
...
async function updateDisplayName(conversationId, newName) {
  try {
    ...
    document.getElementById("chat-with-name").textContent =
      newName || document.getElementById("chat-with-phone").textContent;
    currentConvoPage = 1;
    fetchAndRenderConversations();
  } catch (error) {
...
async function handleManageLabels() {
  ...
  try {
    ...
    const preselectedLabelIds = new Set(
      currentConversationLabels.map((label) => label.id.toString())
    );

    let labelsHtml =
      '<div id="swal-label-list" class="text-left swal2-checkbox-list" style="display: flex; flex-direction: column; align-items: flex-start; max-height: 250px; overflow-y: auto;">';
    allLabels.forEach((label) => {
      const isChecked = preselectedLabelIds.has(label.id.toString())
        ? "checked"
        : "";
    ...
    const fullHtml =
      '<div class="text-sm text-left mb-2">Pilih label untuk percakapan ini:</div>' +
      labelsHtml;
    ...
    const { value: selectedLabelIds } = await Swal.fire({
      ...
      preConfirm: () => {
        const checkboxes = document.querySelectorAll(
          '#swal-label-list input[type="checkbox"]:checked'
        );
...
async function updateConversationLabels(conversationId, labelIds) {
  try {
    ...
    const response = await fetch(
      "/src/api/whatsapp/update_conversation_labels.php",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
async function startConversation() {
  ...
  if (confirmation.isConfirmed) {
    ...
    try {
      const response = await fetch(
        "/src/api/whatsapp/send_live_chat_invitation.php",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (startButton) {
        ...
        startButton.innerHTML =
          '<i class="fas fa-play-circle"></i><span class="hidden sm:inline"></span>';
      }
    }
  }
}
