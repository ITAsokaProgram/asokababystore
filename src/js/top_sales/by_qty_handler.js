document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadTopSalesData() {
    ...
    try {
      ...
      if (pageTitle) {
        pageTitle.textContent = "Top Sales by (Qty)";
      }
    } finally {
  ...
  function setLoadingState(
    ...
    if (isLoading) {
      ...
      if (isExporting) {
        ...
        if (!isPagination) {
          if (summaryTotalQty) summaryTotalQty.textContent = "-";
    ...
    tabel_data.forEach((row, index) => {
      htmlRows += `
                                <tr>
                                    ...
                                    <td class="font-bold">${formatNumber(
                                      row.qty
                                    )}</td>
    ...
    if (tabel_data && tabel_data.length > 0 && summary && isLastPage) {
      htmlRows += `
                <tr class="bg-blue-100 font-bold text-blue-900">
                    ...
                    <td class="">${formatRupiah(summary.total_net_sales)}</td>
  ...
  async function exportToExcel() {
    ...
    try {
      ...
      const params = getUrlParams();
      const title = [["Laporan Top Sales (Quantity)"]];
      const info = [
        ...
        [],
        ["Total Quantity", parseFloat(summary.total_qty) || 0],
      ...
      dataRows.forEach((_, R_idx) => {
        ...
        if (R_idx < dataRows.length - 1) {
          ...
          ws[XLSX.utils.encode_cell({ r: R, c: 2 })].v = "GRAND TOTAL";
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Top Sales Qty");
      const fileName = `Top_Sales_Quantity_${params.tgl_mulai}_sd_${params.tgl_selesai}.xlsx`;
  ...
  async function exportToPDF() {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text("Laporan Top Sales (Quantity)", 14, 22);
      doc.setFontSize(11);
      ...
      const fileName = `Top_Sales_Quantity_${params.tgl_mulai}_sd_${params.tgl_selesai}.pdf`;
      doc.save(fileName);
    } catch (error) {
