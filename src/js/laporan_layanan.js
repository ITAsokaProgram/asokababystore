...
function createTopWrapper($wrapper, $length, $search) {
  const $top = $(
    '<div class="dt-detail-top flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4 mt-4 pl-3 pr-3"></div>'
  );
...
function createBottomWrapper($wrapper, $info, $paginate) {
  const $bottom = $(
    '<div class="dt-detail-bottom flex justify-between items-center mt-4 pl-4 pr-4 text-sm"></div>'
  );
...
function styleInputElements($length, $search) {
  $length
    .find("select")
    .addClass(
      "ml-2 px-2 py-1 border rounded-md focus:ring-2 focus:ring-blue-500"
    );
  $search
...
function stylePaginationButtons() {
  $(".dataTables_paginate a").addClass(
    "px-3 py-1 border rounded-lg text-sm text-gray-700 hover:bg-pink-100 cursor-pointer p-4 mb-3"
  );
...
function renderLaporan(data) {
  ...
  data.forEach((laporan) => {
    ...
    const statusHTML = `<span class="px-3 py-1 rounded-full text-xs font-semibold ${getStatusClass(
      status
    )}">${mapStatusToText(status)}</span>`;

    const detailButton = `<button class="btn-action" onclick='openModal(${JSON.stringify(
      laporan
    )})'>Detail</button>`;
    ...
    if (laporan.is_user_registered) {
      const unreadBadge =
        laporan.unread_count > 0
          ? `<span class="absolute -top-1.5 -right-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-xs font-bold text-white ring-2 ring-white">${laporan.unread_count}</span>`
          : "";
      chatButton = `
                <button 
                    onclick='openChatModal(${laporan.id}, ${JSON.stringify({
        nama: laporan.nama_lengkap,
        handphone: laporan.no_hp,
...
function openChatModal(contactId, contactData) {
  ...
  document.getElementById("chatCustomerPhone").textContent =
    contactData.handphone || "-";
  document.getElementById("chatCustomerSubject").textContent =
    contactData.subject || "-";
  document.getElementById("chatCustomerMessage").textContent =
    contactData.message || "-";
...
async function loadChatConversation(contactId) {
  ...
  try {
    ...
    const response = await fetch(
      `/src/api/customer/contact_us/get_contact_us_conversation.php?contact_us_id=${contactId}`,
      {
...
async function sendChatMessage() {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await fetch(
      `/src/api/customer/contact_us/send_contact_us_message.php`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
function openModal(laporan) {
  ...
  document.getElementById("modalDeskripsi").textContent =
    laporan.message || "-";
  document.getElementById("modalTanggal").textContent = formatTanggal(
    laporan.dikirim
  );
  ...
  fetch(`/src/api/customer/laporan_layanan.php`, {
    ...
    .then((res) => {
      if (res.success) {
        Swal.fire(
          "Berhasil",
          "Laporan telah ditandai sebagai selesai.",
          "success"
        );
        closeModal();
        fetchData();
...
function updateCountStatus(data) {
  ...
  const countInProgress = data.filter(
    (laporan) => laporan.status === "in_progress"
  ).length;
  const countSelesai = data.filter(
    ...
    .then((res) => {
      if (res.success) {
        ...
        const statusCell = document.querySelector(
          `[data-id="${laporan.id}"] .cell-status`
        );
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document
    .getElementById("sendChatMessageBtn")
...
async function kirimBalasanEmail() {
  ...
  if (balasan === "") {
    Swal.fire(
      "Peringatan",
      "Mohon isi pesan balasan terlebih dahulu.",
      "warning"
    );
    return;
  }
  ...
  try {
    ...
    if (error instanceof SyntaxError) {
      ...
      await Swal.fire(
        "Error Kritis",
        "Server memberikan respons yang tidak valid. Cek console untuk detail.",
        "error"
      );
    } else {
