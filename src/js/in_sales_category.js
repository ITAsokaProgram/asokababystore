...
function sendKodeSupp(tanggalAwal, tanggalAkhir, kodeSupp, kodeStore, kategori, filter) {
    document.getElementById("sort-filter").style.display = "none";
    document.getElementById("sort-filter1").style.display = "block";
    showProgressBar()
    const jsonData = {
...
function updateChartEarly(labels, data) {
    ...
    bindChartClick((params) => {
        ...
        sendCategory(startDateInput, endDateInput, kategori, storeCode, valueFilter)
    })
...
function updateChartCat(labels, data) {
    ...
    echartDiagram.setOption({
        ...
        tooltip: {
            ...
            formatter: (params) => {
                if (sortBy === "total") {
                    return `${params.name}: ${params.data.qty} <br> Rp ${params.value.toLocaleString()}`;
                } else {
        ...
        series: [{
            ...
            label: {
                fontSize: 12,
                formatter: (params) => {
                    let p = sortBy === "total" ? params.data.persen_rp : params.data.persen_qty;
                    const persenFix = !isNaN(Number(p)) ? Number(p).toFixed(2) : '0.00';
        ...
        series: [{
            ...
            label: {
                ...
                formatter: function (item) {
                    const percentage = sortBy === "total" ? item.data.persen_rp : item.data.persen_qty;
...
function customizeDataTableLayout(tableId) {
    ...
    $wrapper.find('.dataTables_length label').addClass('text-sm text-gray-600 flex items-center gap-2');
    $wrapper.find('.dataTables_length select').addClass('px-2 py-1 border rounded-lg');

    $wrapper.find('.dataTables_filter label').addClass('text-sm text-gray-600 flex items-center gap-2');
    $wrapper.find('.dataTables_filter input').addClass('px-2 py-1 border rounded-lg');
}
...
function renderTableFromData(dataArray) {
    ...
    const columns = [
        ...
        ...Object.keys(dataArray[0]).map(key => ({
            title: key.toLowerCase() === "barcode" ? "BARCODE" : key.charAt(0).toUpperCase() + key.slice(1).replace(/_/g, ' '),
            data: key
        }))
    ];
    ...
    const dataTable = $(tableId).DataTable({
        ...
        drawCallback: function () {
            ...
            $(`${tableId} thead th`).addClass('bg-pink-500 text-white text-sm font-semibold');
            $(`${tableId} tbody td`).addClass('text-sm text-gray-700 border-b');
            $(`${tableId} tbody tr`).addClass('hover:bg-pink-50 hover:scale-[101%] transition-all duration-150 ease-in-out');
...
$('#sort-by').on('change', function () {
    ...
    if (selected === "total_qty") {
        ...
        sortedChart.sort((a, b) => parseFloat(b.persen_qty) - parseFloat(a.persen_qty));
    } else if (selected === "total") {
        // Sort berdasarkan total (dalam format rupiah), descending
        sortedTable.sort((a, b) => toAngka(b.total) - toAngka(a.total));
        sortedChart.sort((a, b) => parseFloat(b.persen_rp) - parseFloat(a.persen_rp));
    }
