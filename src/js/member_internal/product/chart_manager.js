...
const updateTopMembersPerformance = (memberData) => {
  ...
  lazyLoadList(
    ...
    (member, index) => {
      ...
      return `
        <div class="cursor-pointer ${colors.bg} rounded-xl ${colors.border} border-l-4 ${colors.shadow} shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out group overflow-hidden" 
          ...
          <div class="p-5 relative">
            ...
            <div class="flex items-center justify-between relative">
              <div class="flex items-center min-w-0">
                <!-- Enhanced rank badge -->
                <div class="relative">
                  <div class="w-12 h-12 ${colors.rankBg} ${colors.rankText} rounded-full flex items-center justify-center text-sm font-bold mr-4 shadow-lg group-hover:shadow-xl transition-shadow duration-300">
                    ${rank}
                  </div>
                </div>
            ...
            <div class="mt-4 bg-gray-200 rounded-full h-2">
              <div class="${colors.rankBg} h-full rounded-full transition-all duration-1000 ease-out" 
                   style="width: ${Math.min(100, (member.total_penjualan / Math.max(...memberData.map(m => m.total_penjualan))) * 100)}%"></div>
            </div>
          </div>
        </div>
      `;
...
// Update top 50 non members performance chart/list
const updateTopNonMembersPerformance = (memberData) => {
  window.topNonMembersData = memberData;
  lazyLoadList(
    memberData,
    'top-non-member-performance',
    (member, index) => {
      // Dynamic color scheme based on ranking for non-members
      const getRankColors = (rank) => {
        if (rank === 1) return {
      ...
      // Add special badges for top performers
      ...
      // Generate transaction ID display format
      ...
      return `
        <div class="cursor-pointer ${colors.bg} rounded-xl ${colors.border} border-l-4 ${colors.shadow} shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-in-out group" 
             data-non-member="${member.no_trans}" 
             data-cabang="${member.kd_store}" 
             id="non-member-${member.no_trans}">
          <div class="p-5 relative overflow-hidden">
            <!-- Background decoration -->
            <div class="absolute top-0 right-0 w-28 h-28 bg-white/10 rounded-full -translate-y-12 translate-x-12 group-hover:scale-110 transition-transform duration-500"></div>
            ...
            <div class="flex items-center justify-between relative">
              <div class="flex items-center flex-1">
                ...
                <div class="space-y-1 flex-1 min-w-0">
                  ...
                  <div class="text-sm ${colors.accent} font-semibold bg-white/70 px-2 py-1 rounded-md inline-block">
                    📍 ${member.cabang}
                  </div>
            ...
            <div class="mt-4 bg-gray-200 rounded-full h-2 overflow-hidden">
              <div class="${colors.rankBg} h-full rounded-full transition-all duration-1000 ease-out group-hover:animate-pulse" 
                   style="width: ${Math.min(100, (member.total_penjualan / Math.max(...memberData.map(m => m.total_penjualan))) * 100)}%"></div>
            </div>
          </div>
        </div>
