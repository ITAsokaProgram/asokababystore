...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateTempFooter() {
    ...
    rows.forEach((row) => {
      const qty =
        parseFloat(row.querySelector('[data-field="qty_rec"]').value) || 0;
    ...
    if (grandTotalNetEl)
      grandTotalNetEl.textContent = "Rp " + formatRupiah(grandTotalNet);
    if (ppnEl) ppnEl.textContent = "Rp " + formatRupiah(ppn);
    if (totalPenerimaanEl)
      totalPenerimaanEl.textContent =
        "Rp " + formatRupiahInteger(totalPenerimaan);
  }
  ...
  function updateSelectAllState() {
    ...
    const checkedCheckboxes = tempTableBody.querySelectorAll(
      ".input-cb-temp:checked"
    );
  ...
  async function loadTempTable() {
    ...
    tempTableBody.innerHTML =
      '<tr><td colspan="16" class="text-center p-4"><i class="fas fa-spinner fa-spin"></i> Memuat data keranjang...</td></tr>';
    try {
      const result = await getTempReceiptItems();
      if (result.success && result.items.length > 0) {
        tempTableBody.innerHTML = result.items
          .map((item) => createTempRowHtml(item))
          .join("");
      } else {
        tempTableBody.innerHTML =
          '<tr><td colspan="16" class="text-center p-4 text-gray-500">Keranjang penerimaan masih kosong.</td></tr>';
      }
  ...
  function createTempRowHtml(item) {
    ...
    return `
        <tr data-plu="${item.plu}">
            <td class="p-3 text-center"><input type="checkbox" class="input-cb-temp" value="${
              item.plu
            }"></td>
            ...
            <td class="p-3"><input type="text" value="${formatRupiah(
              avg_cost
            )}" class="input-qty input-disabled" data-field="calc_weighted_avg_cost" disabled></td>
            
            <td class="p-3"><input type="text" value="${formatRupiah(
              hrg_beli
            )}" min="0" class="input-qty input-temp-update input-rupiah" data-field="hrg_beli" inputmode="decimal"></td>
            
            <td class="p-3"><input type="text" value="${formatRupiah(
              ppn
            )}" class="input-qty input-disabled" data-field="ppn" disabled></td>
            <td class="p-3"><input type="text" value="${formatRupiahInteger(
              netto
            )}" class="input-qty input-disabled" data-field="netto" disabled></td>
            <td class="p-3"><input type="text" value="${formatRupiahInteger(
              hb_plus_lainnya
            )}" class="input-qty input-disabled" data-field="calc_hb_plus_lainnya" disabled></td>
            
            <td class="p-3"><input type="text" value="${formatRupiah(
              price
            )}" min="0" class="input-qty input-temp-update input-rupiah" data-field="price" inputmode="decimal"></td>
            
            <td class="p-3"><input type="text" value="${formatRupiahInteger(
              margin
            )}" class="input-qty input-disabled ${marginClass}" data-field="calc_margin" disabled></td>
            <td class="p-3"><input type="text" value="${formatPersen(
              admin_pct
            )}" class="input-qty input-disabled" data-field="kategori_admin_pct" disabled></td>
            <td class="p-3"><input type="text" value="${formatPersen(
              ongkir_pct
            )}" class="input-qty input-disabled" data-field="kategori_ongkir_pct" disabled></td>
            <td class="p-3"><input type="text" value="${formatPersen(
              promo_pct
            )}" class="input-qty input-disabled" data-field="kategori_promo_pct" disabled></td>
            <td class="p-3"><input type="text" value="${formatRupiah(
              biaya_pesanan
            )}" class="input-qty input-disabled" data-field="kategori_biaya_pesanan" disabled></td>
            
            <td class="p-3"><input type="text" value="${formatRupiah(
              total
            )}" class="input-qty input-disabled font-bold text-gray-900" data-field="total" disabled></td>
        </tr>
        `;
  }
  ...
  if (tempTableBody) {
    ...
    tempTableBody.addEventListener(
      "blur",
      (e) => {
  ...
  async function sendUpdateToServer(row, inputElement) {
    ...
    try {
      const updatedItem = await updateTempReceiptItem(
        plu,
        qty_rec,
        hrg_beli,
        price
      ...
      if (updatedItem.success && updatedItem.item) {
        ...
        row.querySelector('[data-field="calc_weighted_avg_cost"]').value =
          formatRupiah(item.calc_weighted_avg_cost);
        row.querySelector('[data-field="ppn"]').value = formatRupiah(item.ppn);
        row.querySelector('[data-field="netto"]').value = formatRupiahInteger(
          item.netto
        );
        row.querySelector('[data-field="calc_hb_plus_lainnya"]').value =
          formatRupiahInteger(item.calc_hb_plus_lainnya);

        const marginInput = row.querySelector('[data-field="calc_margin"]');
        marginInput.value = formatRupiahInteger(margin);

        marginInput.classList.remove(
          "text-red-600",
          "text-green-600",
          "text-gray-900",
          "font-bold"
        ...
        row.querySelector('[data-field="kategori_admin_pct"]').value =
          formatPersen(item.kategori_admin_pct);
        row.querySelector('[data-field="kategori_ongkir_pct"]').value =
          formatPersen(item.kategori_ongkir_pct);
        row.querySelector('[data-field="kategori_promo_pct"]').value =
          formatPersen(item.kategori_promo_pct);
        row.querySelector('[data-field="kategori_biaya_pesanan"]').value =
          formatRupiah(item.kategori_biaya_pesanan);
        ...
        throw new Error(
          updatedItem.message || "Data item tidak diterima dari server."
        );
      }
    } catch (error) {
      ...
      Swal.fire(
        "Update Gagal",
        `Gagal menyimpan data untuk PLU ${plu}: ${error.message}`,
        "error"
      );
    } finally {
      if (inputElement) {
  ...
  if (deleteSelectedButton) {
    deleteSelectedButton.addEventListener("click", async () => {
      const checkboxes = tempTableBody.querySelectorAll(
        ".input-cb-temp:checked"
      );
