document.addEventListener("DOMContentLoaded", () => {
    ...
    function renderTable(rows, offset) {
        ...
        rows.forEach((row, index) => {
            let wilayahBadges = '-';
            if (row.wilayah) {
                const wilayahArray = row.wilayah.split(',').map(w => w.trim()).filter(w => w !== "");
                const maxDisplay = 3; 
                if (wilayahArray.length <= maxDisplay) {
                    wilayahBadges = wilayahArray.map(w => 
                        `<span class="inline-block px-2 py-0.5 mb-1 text-[10px] rounded bg-blue-50 text-blue-600 border border-blue-100">${w}</span>`
                    ).join(' ');
                } else {
                    const visibleItems = wilayahArray.slice(0, maxDisplay);
                    const hiddenItems = wilayahArray.slice(maxDisplay);
                    const remainingCount = hiddenItems.length;
                    const visibleBadges = visibleItems.map(w => 
                        `<span class="inline-block px-2 py-0.5 mb-1 text-[10px] rounded bg-blue-50 text-blue-600 border border-blue-100">${w}</span>`
                    ).join(' ');
                    const moreBadge = `<span class="inline-block px-2 py-0.5 mb-1 text-[10px] rounded bg-gray-100 text-gray-600 border border-gray-200 font-medium cursor-help" title="${hiddenItems.join(', ')}">+${remainingCount} Lainnya</span>`;
                    wilayahBadges = visibleBadges + ' ' + moreBadge;
                }
            }
