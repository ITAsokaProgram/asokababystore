export const renderTableReview = (data, offset) => {
  ...
  setTimeout(() => {
    ...
    data.forEach((item, index) => {
      ...
      row.className =
        "hover:bg-gray-50 transition-all duration-200 border-b border-gray-100 hover:-translate-y-0.5 hover:shadow-sm";

      let actionButton = "";
      if (!item.sudah_terpecahkan) {
        actionButton = `
                <button 
                    onclick="openIssueHandlingModal(${
                      item.id
                    }, ${JSON.stringify({
          nama: item.nama,
          handphone: item.hp,
      ...
      row.innerHTML = `
                <td class="px-4 py-3 text-sm text-gray-900 font-medium transition-all duration-200">${
                  offset + index + 1
                }</td>
                <td class="px-4 py-3 text-sm text-gray-700 transition-all duration-200">${
                  item.hp
                }</td>
                <td class="px-4 py-3 text-sm text-gray-900 font-medium transition-all duration-200 truncate">${
                  item.nama
                }</td>
                ...
                <td class="px-4 py-3 text-sm text-gray-700 transition-all duration-200">
                    <div class="flex items-center space-x-2">
                        ...
                        <span>${new Date(item.tanggal).toLocaleDateString(
                          "id-ID"
                        )}</span>
                    </div>
                </td>
       ...
       title="Lihat Struk">
        ...
        <i class="fas fa-external-link-alt text-[10px] opacity-0 group-hover:opacity-100 transition-opacity"></i>
                ...
                <td class="px-4 py-3 text-sm transition-all duration-200 relative">
                    <div class="flex items-center space-x-2">
                        ${
                          item.rating <= 3 || item.detail_review_id != null
                            ? chatButton
                            : ""
                        }
                        ${actionButton}
