...
function fetchDataTotalTrans(store, startDate, endDate) {
  ...
  fetch(
    "https://asokababystore.com/src/api/transaction/get_transaction_member",
    {
      method: "POST",
      headers: {
...
function fetchDataTotalTransNonMember(store, startDate, endDate) {
  ...
  document.getElementById("progressOverlay").classList.remove("hidden");
  fetch(
    "https://asokababystore.com/src/api/transaction/get_transaction_non_member",
    {
      method: "POST",
...
function updateCharts(data) {
  ...
  const totalPromo = data.data.reduce(
    (sum, item) => sum + (parseInt(item.total_barang_terjual_promo) || 0),
    0
  );
  const totalNonPromo = data.data.reduce(
    (sum, item) => sum + (parseInt(item.total_barang_terjual_non_promo) || 0),
    0
  );
...
function updateChartsPie(data) {
  const totalMember = data.data.reduce(
    (sum, item) => sum + (parseInt(item.transaksi_member) || 0),
    0
  );
  const totalNonMember = data.data.reduce(
    (sum, item) => sum + (parseInt(item.transaksi_non_member) || 0),
    0
  );
...
function updateBarCharts(data) {
  ...
  const nonMemberTransactions = data.data.map(
    (item) => item.transaksi_non_member
  );
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        params.forEach((param) => {
          ...
          result += `${param.marker} ${param.seriesName}: <strong>${
            param.value
...
document.getElementById("cek").addEventListener("click", (e) => {
  ...
  // AMBIL VALUE DARI DROPDOWN YANG BARU SAJA KITA BUAT DINAMIS
  const storeCode = document.getElementById("cabang").value;

  const startDate = $("#filterRangeDateStart").val();
  const endDate = $("#filterRangeDateEnd").val();

  // Validasi sederhana agar user memilih cabang
  if (storeCode === "none") {
    Swal.fire({
      icon: "warning",
      title: "Peringatan",
...
function loadStores() {
  ...
  fetch(url, {
    ...
    .catch((error) => {
      ...
      const select = document.getElementById("cabang");
      select.innerHTML = '<option value="none">Error koneksi</option>';
    });
}
