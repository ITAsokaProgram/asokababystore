document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (pageSubtitle) {
        ...
        pageSubtitle.textContent = `Laporan Return Out Expired Periode ${params.tgl_mulai} s/d ${params.tgl_selesai} - ${storeName}`;
        if (pageTitle) {
      ...
      if (pageTitle) {
        pageTitle.textContent = "Laporan Return Out (Expired)";
      }
    } finally {
  ...
  async function exportToExcel() {
    ...
    try {
      ...
      const params = getUrlParams();
      const title = [["Laporan Return Out (Expired)"]];
      const info = [
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Return Expired");
      const fileName = `Return_Out_Exp_Produk_${params.tgl_mulai}_sd_${params.tgl_selesai}.xlsx`;
  ...
  async function exportToPDF() {
    ...
    try {
      ...
      doc.setFontSize(18);
      doc.text("Laporan Return Out (Expired)", 14, 22);
      doc.setFontSize(11);
