...
try {
    // Opsional: Hapus data lama agar bersih
    ...
    for ($i = 1; $i <= $total_data; $i++) {
        ...
        // --- LOGIKA NOMOR DOKUMEN (Multi Value) ---
        // Kita ambil 1 sampai 3 jenis dokumen secara acak untuk digabung
        $doc_count = rand(1, 3);
        $docs = [];
        for ($d = 0; $d < $doc_count; $d++) {
            $type = rand(0, 2);
            // Pakai uniqid agar benar-benar unik dan tidak duplicate key error
        ...
        // Gabungkan dengan koma + spasi
        $nomor_dokumen = implode(", ", $docs);

        // --- LOGIKA PIC (Multi Value) ---
        // Ambil 1 sampai 3 orang dari master pic secara acak
        $shuffled_pic = $list_pic_master;
        shuffle($shuffled_pic); // Acak urutan
        $pic_count = rand(1, 3); // Ambil 1, 2, atau 3 orang
        ...
        // Bind Param (Semua String "s")
        $types = str_repeat("s", 19);

        mysqli_stmt_bind_param(
            $stmt,
            $types,
            $nomor_dokumen, // Isinya: "SKP001, KWT002"
            $pic,           // Isinya: "Annisa, Dewi"
            $supplier,
            $kode_cabang,
            $nama_cabang,
            $periode,
