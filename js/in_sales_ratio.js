...
$(document).ready(function () {
  ...
  $("#cabang").on("change", function () {
    ...
    $("#kd_store").val(selectedValue);
  });
});
function loadCabangOptions() {
  $.ajax({
    url: "../../api/option/get_cabang.php",
    type: "GET",
    dataType: "json",
    success: function (response) {
      if (response.success) {
        ...
        $.each(response.data, function (index, item) {
          allStoreIds.push(item.Kd_Store);
          optionsHtml += `<option value="${item.Kd_Store}">${item.Nm_Alias}</option>`;
        });
        var allIdsString = allStoreIds.join(",");
        optionsHtml += `<option value="${allIdsString}">SEMUA CABANG</option>`;
        $("#cabang").html(optionsHtml);
        $("#cabang").prop("selectedIndex", 0).trigger("change");
      } else {
        console.error("Gagal mengambil data cabang: " + response.message);
        $("#cabang").html(
    ...
    error: function (xhr, status, error) {
      console.error("Error AJAX Cabang:", error);
      $("#cabang").html('<option value="" disabled>Error koneksi</option>');
    },
  });
}
document.addEventListener("DOMContentLoaded", function () {
...
function sendKodeSupp1(page) {
  ...
  formDataToTable.append(
    "kd_store",
    document.querySelector("#kd_store")?.value
  );
...
function updateBarChart(labels, data, table) {
  ...
  var seriesData = kods.map((kd) => ({
    ...
    data: tanggal.map((tgl) => {
      let foundItem = newData.find(
        (item) => item.tanggal === tgl && item.kode_supp === kd
      );
      console.log("found", foundItem);
      return foundItem
        ? { value: foundItem.Qty, precentage: foundItem.precentage }
        : 0;
    }),
    itemStyle: {
      color: colorMap[kd],
    ...
    tooltip: {
      ...
      formatter: function (params) {
        ...
        let details = params
          .filter((p) => p.value > 0)
          .map(
            (p) => `● ${p.seriesName}: <b>${p.value}</b> (${p.data.precentage})`
          )
          .join("<br>");
...
function updateTable1(data) {
  ...
  data.forEach((row, index) => {
    ...
    let itemNumber = (currentPage - 1) * rowsPerPage + index + 1;
    cell0.textContent = itemNumber;
    ...
    .then((data) => {
      ...
      doc.autoTable({
        ...
        didDrawPage: function (data) {
          ...
          doc.text(
            "Lb 5, Jl. Utan Jati Blok Lb 5 No.9, RT.10/RW.12, Kalideres, West Jakarta City, Jakarta 11840",
            45,
            20
          );
          ...
          doc.setFontSize(12);
          doc.text(
            `Laporan Data Penjualan Sales Cabang ${cabangText} Supplier ${selectedKode}`,
            15,
            45
          );
        },
      ...
      doc.save(
        `Data_Penjualan_Cabang_${cabangText}_Supplier_${selectedKode}.pdf`
      );
    })
    .catch((error) => console.error("Error fetching data:", error));
}
